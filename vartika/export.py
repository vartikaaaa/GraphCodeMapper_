...
from vartika.graph import GraphRepresentation, GraphType
...
class GraphExporter:
    ...
    def export_graph_as_graphml(graph, export_name, export_dir):
        nx.write_graphml(graph, export_dir + '/' + 'graphcodemapper-' + export_name + '.graphml')
...
class TableExporter:
    ...
    def export_statistics_and_metrics_as_file(
        ...
        file = open(export_dir + '/' + 'graphcodemapper-' + 'statistics-metrics' + '.txt', 'w', encoding="utf-8")
...
class JSONExporter:
    ...
    def export_statistics_and_metrics(
        ...
        if bool(statistics) or bool(overall_metric_results):
            with open(export_dir + '/' + 'graphcodemapper-' + 'statistics-and-metrics' + '.json', 'w', encoding="utf-8") as file:
    ...
    def export_d3_force_directed_graph(graph_representations, statistics: Dict[str, Any], overall_metric_results: Dict[str, Any], analysis, export_dir):
        ...
        for _, graph_representation in graph_representations.items():
            ...
            target_export_file_path = export_dir + \
                '/' + 'graphcodemapper-' + graph_representation.graph_type.name.lower() + '-data.json'
        ...
        app_config = {
            ...
            'Graphcodemapper_version': analysis.Graphcodemapper_version,
        ...
        d3_js_string = d3_js_string.replace('-', '_')  
